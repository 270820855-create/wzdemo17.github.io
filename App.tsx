      ...
      <div className={`flex flex-col h-full w-full z-10 transition-all duration-500 ease-in-out ${isImmersive ? 'opacity-0 scale-105 pointer-events-none blur-sm' : 'opacity-100 scale-100'}`}>
        ...
        <div className="flex flex-1 overflow-hidden relative w-full">
          ...
          <div className={`
              ...
              md:relative md:translate-x-0 md:z-0 md:border-none
            ...
            <div className="w-full max-w-7xl mx-auto px-4 sm:px-8 md:px-12 lg:px-20 pb-40">
              ...
              <div className="mb-8 flex flex-col items-center justify-center w-full relative">
                  <div className="relative mb-3 group cursor-default">
                      {/* Background Stroke */}
                      <h2 className="text-5xl md:text-7xl font-anime font-black text-white tracking-tighter leading-none relative z-10" 
                          style={{ WebkitTextStroke: '3px black' }}>
                        {activeCategory === 'ALL' ? t('app.dashboard') : t(`category.${activeCategory}`)}
                      </h2>
                      {/* Foreground Offset */}
                      <h2 className="text-5xl md:text-7xl font-anime font-black text-black tracking-tighter leading-none absolute top-1 left-1 z-0 opacity-20">
                        {activeCategory === 'ALL' ? t('app.dashboard') : t(`category.${activeCategory}`)}
                      </h2>
                      {/* Japanese Subtitle */}
                      <span className="absolute -bottom-4 -right-8 bg-black text-white text-xs md:text-sm px-2 py-0.5 font-bold transform -rotate-3 z-20">
                          {activeCategory === 'ALL' ? 'ダッシュボード' : 'カテゴリー'}
                      </span>
                      {/* Underline Slash */}
                      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-[120%] h-6 bg-neon-green/80 -z-0 -rotate-2 skew-x-12"></div>
                  </div>
                  
                  <div className="flex items-center gap-3 mt-4">
                      <span className="text-xs md:text-sm font-mono font-bold text-gray-500 uppercase bg-white px-3 py-1 border-2 border-black shadow-[3px_3px_0_#000]">
                        // SYSTEM: {activeCategory}
                      </span>
                      <div className="w-8 h-[2px] bg-black"></div>
                      <span className="text-xs md:text-sm font-black bg-jinx-pink text-white px-3 py-1 border-2 border-black shadow-[3px_3px_0_#000] transform skew-x-12">
